...
import './ProfileEmloyee.css';

const ProfileEmloyee = () => {
  const [userData, setUserData] = useState(null);
  const [isLoading, setIsLoading] = useState(true);
  const [isEditing, setIsEditing] = useState(false);  const [formData, setFormData] = useState({
    user_name: '',
    user_email: '',
    // Nous gardons uniquement les champs qui existent dans la base de données
  });
  ...
  return (
    <div className="profile-container">
      ...
      <div className="profile-content">
        <div className="profile-main">          <div className="profile-avatar">
            <div className="avatar-placeholder">
              {userData?.user_name?.charAt(0)}
            </div>
            <h2>{userData?.user_name}</h2>
            <p className="job-title">{userData?.role || 'User'}</p>
          </div>{isEditing ? (
              ...
              <div className="detail-section">
                ...
                <div className="detail-row">
                  ...
                  <div className="detail-item">
                    ...
                    <span className="detail-value">{userData?.role || 'User'}</span>
                  </div>
                </div>
              </div>
            </div>          )}
